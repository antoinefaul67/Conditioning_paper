...
class GMC_bijector(tfb.Bijector):
    ...
    def _inverse(self, u_mat):
        ...
        #cdf_funct = lambda x:tfd.MixtureSameFamily(mixture_distribution=tfd.Categorical(logits=self.logits),
                              #components_distribution=tfd.Normal(loc=tf.transpose(self.mu_vectors),
                                                                # scale=tf.transpose(self.std_vectors))).cdf(x)
        #x_mat = utl.icdf_numerical(u_mat, cdf_funct)
        x_mat = self.gmm_icdf_parallel(u_mat, self.logits, tf.transpose(self.mu_vectors),
                                       tf.transpose(self.std_vectors))
        return x_mat
    ...
    def gmm_icdf_parallel(self, u_mat, logit, mu_T, std_T):
        ...
        obj_func = lambda x: tfd.MixtureSameFamily(mixture_distribution=tfd.Categorical(logits=logit),
                                                   components_distribution=tfd.Normal(loc=mu_T, scale=std_T)).cdf(
            x) - u_mat
        ...
        x_mat = tfp.math.find_root_chandrupatla(obj_func, low=lb, high=ub, position_tolerance=1e-12, value_tolerance=1e-12)[0]
        #x_mat = tfp.math.find_root_secant(obj_func, initial_position=tf.constant([0]), position_tolerance=1e-12,
                                          #value_tolerance=1e-12)[0]
        #breakpoint()
        #obj_func_prim = lambda x:tfd.MixtureSameFamily(mixture_distribution=tfd.Categorical(logits=logit),
                                                   #components_distribution=tfd.Normal(loc=mu_T, scale=std_T)).prob(x)
        #x_mat = utl.newton(f=obj_func,Df=obj_func_prim, x0=0, epsilon=1e-7, max_iter=1000)

...
class GMC:
    def __init__(self, n_dims, n_comps, param_vec=None):
        ...
        if self.params is not None:
            assert tf.size(
                self.params) == self.total_trainable_params, 'the supplied parameter vector is not commensurate with the n_dims, and n_comps'
            self.params = param_vec
    ...
    def distribution(self):
        ...
        # Instantiating the gmc distribution as a transformed distribution
    ...
    def identifiability_prior(self):
        ...
        log_prior_1 = tfd.MultivariateNormalDiag(loc=tf.zeros(self.ndims),
                                                 scale_diag=1E-1 * tf.ones(self.ndims)).log_prob(vec1)
        log_prior_2 = tfd.MultivariateNormalDiag(loc=tf.ones(self.ndims),
                                                 scale_diag=1E-1 * tf.ones(self.ndims)).log_prob(vec2)
...
class GMCM:
    def __init__(self,
        ...
        # verifying if the gmc parameterization is consistent with the specified gmcm
        if self.gmc:
        ...
        if self.preproc_transform:
            ...
            gmcm_dist = tfd.TransformedDistribution(distribution=self.gmc.distribution.distribution,
                                                    bijector=tfb.Chain([self.marg_bijector,
                                                                        self.gmc.distribution.bijector]))
            #gmcm_dist = tfd.TransformedDistribution(distribution=self.gmc.distribution.distribution,
                                                    #bijector=self.gmc.distribution.bijector)
        return gmcm_dist

   # def learn_marginals_2(self):
        #marg_dist_list = []

        #for j in range(self.ndims):
            #input_vector = self.data_trn[:, j]
            #kde1 = gaussian_kde(input_vector)

            # Define the CDF function for the KDE
           # kde.cdf = lambda x: quad(kde, -np.inf, x)[0]
            #def kde_cdf(x):
                #return tf.convert_to_tensor(quad(kde1, -np.inf, x)[0], dtype="float64")

            # Define the log PDF function for the KDE
            #def kde_log_pdf(x):
                #return tf.convert_to_tensor(kde1.logpdf(x))
            ...
            #info_dict = {
                #'cdf': np.vectorize(kde_cdf),
                #'log_pdf': np.vectorize(kde_log_pdf),
                #'lb': tf.convert_to_tensor(lb, dtype=tf.float32),
                #'ub': tf.convert_to_tensor(ub, dtype=tf.float32)
            #}
    ...
    def learn_marginals_2(self):
        ...
        for j in range(self.ndims):
            ...
            def kde_cdf(x):
                ...
                result, _ = quad(kde1, -np.inf, x_np)
                return tf.convert_to_tensor(result, dtype=tf.float64)

            # Define the log PDF function for the KDE
            def kde_log_pdf(x):
                x_np = x.numpy() if isinstance(x, tf.Tensor) else x
                result = kde1.logpdf(x_np)
                return tf.convert_to_tensor(result, dtype=tf.float64)

                # Define the bounds
            lb = np.min(input_vector) - 3 * np.std(input_vector)
            ub = np.max(input_vector) + 3 * np.std(input_vector)

                # Creating a dictionary containing necessary information about each marginal distribution
            info_dict = {
                'cdf': np.vectorize(kde_cdf),
                'log_pdf': np.vectorize(kde_log_pdf),
                'lb': tf.convert_to_tensor(lb, dtype=tf.float64),
                'ub': tf.convert_to_tensor(ub, dtype=tf.float64)
            }
        ...
        #for j in range(self.ndims):
            #input_vector = self.data_trn[:, j]
            #kernel = gaussian_kde(input_vector)
            #kernel = KDEUnivariate(input_vector)
            #kernel.fit(kernel='gau', bw='normal_reference')
            #input_vector = input_vector.reshape(-1, 1)
            #fi = lambda x: kernel.integrate_box_1d(low=float('-inf'),high=x)
            #fi = np.vectorize(fi)
            #marg_gmm_tfp = tfd.Empirical(samples=input_vector)
            #cdf = lambda x:tuple(ndtr(np.ravel(item - kernel.dataset) / kernel.factor).mean()
                        #for item in x)
        ...
        for j in range(self.ndims):
            ...
            marg_gmm_tfp = tfd.MixtureSameFamily(
                ...
                components_distribution=tfd.Normal(loc=marg_gmm_obj.means_.flatten().astype('float32'),
                                                   scale=marg_gmm_obj.covariances_.flatten().astype(
                                                       'float32') ** 0.5), )
    ...
    def fit_dist_IFM(self,
        ...
        if method == "TGMM":
            gmc_obj = GMC(self.ndims, self.ncomps)
            u_lat_trn = norm.ppf(u_mat_trn)
            gmm = GaussianMixture(n_components=n_comps).fit(u_lat_trn)
            alphas = gmm.weights_.astype('float32')
            mus = gmm.means_.astype('float32')
            covs = gmm.covariances_.astype('float32')
            #covs = [tf.constant(covs[0])]
            #covs = [tf.constant(covs)]
            params = utl.gmm_params2vec(n_dims=self.ndims, n_comps=self.ncomps, alphas=alphas, mu_vectors=mus, cov_matrices=covs)
            #mus_flattened = mus.flatten()
            #alphas_flattened = alphas.flatten()
            #covs_flattened = covs.flatten()

            # Concatenate the arrays
            #concatenated_array = np.concatenate((mus_flattened, alphas_flattened, covs_flattened))

            # Create a TensorFlow variable
            #variable = tf.Variable(concatenated_array, dtype=tf.float32)
            gmc_obj.params = params
        ...
        if method == "GC":
            ...
            gmc_obj = GMC(self.ndims, self.ncomps)
            u_lat_trn = norm.ppf(u_mat_trn)
            mus = np.mean(u_lat_trn, axis=0).astype('float32')
            covs = np.cov(u_lat_trn, rowvar=False).astype('float32')
            alphas = np.array([1],dtype="float32")
            covs = [tf.constant(covs)]
            params = utl.gmm_params2vec(n_dims=self.ndims, n_comps=self.ncomps, alphas=alphas, mu_vectors=mus, cov_matrices=covs)
            #mus_flattened = mus.flatten()
            #alphas_flattened = alphas.flatten()
    ...
    def fit_copula(self,
        ...
        # fitting the gmc density
        neg_ll_trn, neg_ll_vld, param_history = gmc_obj.fit_dist(data_trn,
                                                                 n_comps,
                                                                 u_mat_valid=data_vld,
                                                                 optimizer=optimizer,
                                                                 max_iters=max_iters,
